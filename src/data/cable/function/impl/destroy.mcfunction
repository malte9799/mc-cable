...
execute as @e[type=item_display,dx=0,tag=cable.network,predicate=cable:same_type] run function ./destory/network:
	...
	execute \
		as @s[tag=cable.cable] \
		positioned ^ ^ ^1 \
		if entity @e[limit=1,type=item_display,dx=0,tag=cable.cable,predicate=cable:same_type,predicate=cable:opposite_direction] \
		as @e[type=item_display,dx=0,tag=cable.network,tag=!cable.checked,predicate=cable:same_type] \
		run function ./destory/network
