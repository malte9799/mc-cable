...
execute as @n[type=glow_item_frame,tag=cable.block,distance=..10] at @s align xyz run function ./placed_cable_2 with entity @s Item.components."minecraft:custom_data":
...
execute if entity @e[limit=1,type=glow_item_frame,distance=..10,tag=cable.block] run function ./placed_cable